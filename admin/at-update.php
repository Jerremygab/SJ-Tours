...
//    $offering = filter_var($offering, FILTER_SANITIZE_STRING);
   ...
   if(!empty($image_01)){
      if($image_size_01 > 2000000){
         ...
         $update_image_01 = $conn->prepare("UPDATE `attractions` SET img1 = ? WHERE id = ?");
   ...
   if(!empty($image_02)){
      if($image_size_02 > 2000000){
         ...
         $update_image_02 = $conn->prepare("UPDATE `attractions` SET img2 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `attractions` SET img3 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `attractions` SET img4 = ? WHERE id = ?");
         $update_image_03->execute([$image_03, $pid]);
         move_uploaded_file($image_tmp_name_03, $image_folder_03);
         unlink('../img/tourist/'.$old_image_03);
         $message[] = 'image 03 updated successfully!';
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `attractions` SET img5 = ? WHERE id = ?");
         $update_image_03->execute([$image_03, $pid]);
         move_uploaded_file($image_tmp_name_03, $image_folder_03);
         unlink('../img/tourist/'.$old_image_03);
         $message[] = 'image 03 updated successfully!';
...
<section class="update-product">
   ...
   <?php
      ...
      $select_contents = $conn->prepare("SELECT * FROM `attractions` WHERE id = ?");
