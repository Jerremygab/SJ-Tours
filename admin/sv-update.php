...
//    $offering = filter_var($offering, FILTER_SANITIZE_STRING);
   ...
   $update_content = $conn->prepare("UPDATE `souvenirs` SET place_name = ?, details = ?, location = ?, fb_link = ?, ig_link = ?, 
   ...
   if(!empty($image_01)){
      if($image_size_01 > 2000000){
         ...
         $update_image_01 = $conn->prepare("UPDATE `souvenirs` SET img1 = ? WHERE id = ?");
   ...
   if(!empty($image_02)){
      if($image_size_02 > 2000000){
         ...
         $update_image_02 = $conn->prepare("UPDATE `souvenirs` SET img2 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `souvenirs` SET img3 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `souvenirs` SET img4 = ? WHERE id = ?");
         $update_image_03->execute([$image_03, $pid]);
         move_uploaded_file($image_tmp_name_03, $image_folder_03);
         unlink('../img/tourist/'.$old_image_03);
         $message[] = 'image 03 updated successfully!';
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `souvenirs` SET img5 = ? WHERE id = ?");
         $update_image_03->execute([$image_03, $pid]);
         move_uploaded_file($image_tmp_name_03, $image_folder_03);
         unlink('../img/tourist/'.$old_image_03);
         $message[] = 'image 03 updated successfully!';
...
<section class="update-product">
   ...
   <?php
      ...
      $select_contents = $conn->prepare("SELECT * FROM `souvenirs` WHERE id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="text" name="web_link"  class="box" maxlength="100" placeholder="enter WEB Link" value="<?= $fetch_contents['web_link']; ?>">
      <span>update IG Link</span>
      <input type="text" name="ig_link"  class="box" maxlength="100" placeholder="enter IG Link" value="<?= $fetch_contents['ig_link']; ?>">
      ...
      <div class="flex-btn">
         ...
         <a href="sv-content.php" class="option-btn">go back</a>
      </div>
   </form>
