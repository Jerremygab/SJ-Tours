...
if(isset($_POST['update'])){
   ...
   $diets = $_POST['diets'];
   $diets = filter_var($diets, FILTER_SANITIZE_STRING);
   $features = $_POST['features'];
   $features = filter_var($features, FILTER_SANITIZE_STRING);
   $offering = $_POST['offering'];
   $offering = filter_var($offering, FILTER_SANITIZE_STRING);

   $update_content = $conn->prepare("UPDATE `gastronomic_exp` SET place_name = ?, rate = ?, details = ?, location = ?, fb_link = ?, ig_link = ?, 
                                                                   web_link = ?, gmail_link1 = ?, gmail_link2 = ?, phone_link1 = ?, phone_link2 = ?, map_link = ?, 
                                                                   embedded_map = ?, diets = ?, features = ?, offering = ?
                                                                   WHERE id = ?");
   $update_content->execute([$name, $price, $details,$location,$fb_link,$ig_link,$web_link,$gmail_link1,$gmail_link2,$phone_link2, $map_link,$embedded_map,$diets, $features,$offering, $pid]);
   ...
   if(!empty($image_01)){
      if($image_size_01 > 2000000){
         ...
         $update_image_01 = $conn->prepare("UPDATE `gastronomic_exp` SET img1 = ? WHERE id = ?");
   ...
   if(!empty($image_02)){
      if($image_size_02 > 2000000){
         ...
         $update_image_02 = $conn->prepare("UPDATE `gastronomic_exp` SET img2 = ? WHERE id = ?");
   ...
   if(!empty($image_03)){
      if($image_size_03 > 2000000){
         ...
         $update_image_03 = $conn->prepare("UPDATE `gastronomic_exp` SET img3 = ? WHERE id = ?");
   ...
   if(!empty($image_04)){
      if($image_size_04 > 2000000){
         ...
         $update_image_04 = $conn->prepare("UPDATE `gastronomic_exp` SET img4 = ? WHERE id = ?");
   ...
   $image_folder_53 = '../img/tourist/'.$image_05;

   if(!empty($image_05)){
      if($image_size_05 > 2000000){
         $message[] = 'image size is too large!';
      }else{
         $update_image_05 = $conn->prepare("UPDATE `gastronomic_exp` SET img5 = ? WHERE id = ?");
...
<section class="update-product">
   ...
   <?php
      ...
      $select_contents = $conn->prepare("SELECT * FROM `gastronomic_exp` WHERE id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="text" name="embedded_map"  class="box" maxlength="100" placeholder="enter Embedded Map" value="<?= $fetch_contents['embedded_map']; ?>">
      <span>update Dining services</span>
      <input type="text" name="diet"  class="box" maxlength="100" placeholder="enter Property Amenities" value="<?= $fetch_contents['diets']; ?>">
      <span>update Service Features</span>
      <input type="text" name="features"  class="box" maxlength="100" placeholder="enter Room Features" value="<?= $fetch_contents['features']; ?>">
      <span>update Offerings</span>
      <input type="text" name="offerings"  class="box" maxlength="100" placeholder="enter Room Types" value="<?= $fetch_contents['offering']; ?>">
      ...
      <div class="flex-btn">
         ...
         <a href="gs-content.php" class="option-btn">go back</a>
      </div>
   </form>
